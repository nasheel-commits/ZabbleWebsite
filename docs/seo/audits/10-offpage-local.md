# Audit 10 — Off-Page, Local SEO & Competitive

- **Session:** 10
- **Branch:** seo/10-offpage-local
- **Owner:** S10 (off-page/local strategist)
- **Status:** done
- **Date:** 2026-06-04
...
> **Numbering reconciliation.** The goal that drove this work labelled it "04
> / seo/04-offpage-local". The **binding** `00-conventions.md` §1 assigns off-page
> /local/competitive to **Session 10**, and `seo/04-*` is Site Architecture
> (already a live branch). To avoid colliding with that session, this work runs as
> **Session 10** (`seo/10-offpage-local`, this file, `_evidence/10/`). Deliverables
> requested by the goal are all present: `targets/competitors.md`, this audit's
> backlink + digital-PR plan, `targets/local-seo.md`, the sameAs hand-off in
> `status.md`, and raw exports in `_evidence/10/`.
...
`_evidence/10/README.note.md`. Total spend **~$0.18**.
...
| # | Finding | Evidence |
|---|---------|----------|
| F1 | **zabble.org has zero organic keywords** in SA — a true new-domain baseline. Off-page authority = ~0. | `[E: 10/labs-domain-rank-overview__competitors__za.json]` |
| F2 | The SERP is a **low-volume, winnable niche**: leader `dvt.co.za` 380 kw / 9,675 ETV; `synthesis.co.za` 152 / 6,731; most peers 40–150 kw / a few-hundred ETV. No unassailable incumbent on the long tail. | `[E: 10/labs-domain-rank-overview__*__za.json]` |
| F3 | The **dominant search intent is geo-modified** ("software/IT/tech company + {city}"): Durban 720, Pretoria 480, Cape Town 480, Sandton 260, Joburg 210, Bloemfontein 170. | `[E: 10/labs-domain-intersection__scrums-x-dvt__za.json; 10/labs-ranked-keywords__scrums__za.json]` |
| F4 | The SEO-mature peer (`scrums.com`) wins via **three repeatable assets**: programmatic `/software-development-company/{city}` local pages, a `/top-100-fintech-platforms-in-africa` linkbait directory (ranks `investec` 60,500; `yoco` 27,100), and educational `/blog` + tool guides. | `[E: 10/labs-ranked-keywords__scrums__za.json]` |
| F5 | **Directories rank for Zabble's terms** (`clutch.co`, `goodfirms.co`, `designrush.com`) — these are citation/PR **targets**, not rivals. SA media (`businesstech`, `mybroadband`, `bizcommunity`) are PR placement surfaces. | `[E: 10/labs-serp-competitors__core-cluster__za.json]` |
| F6 | **Local Maps is an opening:** comparable bespoke consultancies hold only modest review counts (BBD 4.6/249, Entelect 4.7/179, CPI 4.7/157); few bespoke peers run a strong GBP. | `[E: 10/business-listings__software-company-jhb.json; 10/business-listings__software-company-cpt.json]` |
| F7 | **Backlink data is gated** — Backlinks API not subscribed (`40204`). Competitor referring-domain profiles couldn't be exported. | `[E: 10/README.note.md]` |
...
|---|-----|----------------|--------------|----------|
| 1 | P0 | Build NAP-consistent citation/entity set (GBP + 10 directories) | `targets/local-seo.md` §3–4 / user + S10 | F1, F6 |
...
| 6 | P1 | Enforce safe anchor distribution on influenced links | §5 R4 / S10 | — |
| 7 | P1 | Hold believable link velocity; monitor monthly | §5 R5 / S10 | — |
...
- **S07 / S08 (AEO/GEO entity):** stand up the **`sameAs` / brand-citation set** —
  full list handed via `status.md` (S10 → S07/S08 note). Use the NAP boilerplate
...
- **S05 (keywords):** size the metro/geo cluster + module-level commercial terms;
  S10's seeds are in `targets/competitors.md` §3.
...
| File | Proves |
|------|--------|
| `_evidence/10/labs-serp-competitors__core-cluster__za.json` | The SA SERP competitor set (77 domains) + directories to discount. |
| `_evidence/10/labs-domain-rank-overview__competitors__za.json` | zabble.org = zero organic keywords (new-domain baseline). |
| `_evidence/10/labs-domain-rank-overview__<domain>__za.json` (×10) | Per-competitor SA visibility (KW count, ETV, positions). |
| `_evidence/10/labs-domain-intersection__scrums-x-dvt__za.json` | Contested commercial keyword space (gap targets). |
| `_evidence/10/labs-ranked-keywords__scrums__za.json` | Competitor content/link-asset playbook (local pages, linkbait directory, guides). |
| `_evidence/10/business-listings__software-company-jhb.json` | Johannesburg Maps/GBP benchmark. |
| `_evidence/10/business-listings__software-company-cpt.json` | Cape Town Maps/GBP benchmark. |
| `_evidence/10/backlinks-bulk-ranks__competitors__za.json` + `backlinks-summary__*.json` | Backlinks API gated (`40204`) — blocker B5. |
| `_evidence/10/README.note.md` | Full method, request bodies, modes, and per-call costs (~$0.18 total). |
